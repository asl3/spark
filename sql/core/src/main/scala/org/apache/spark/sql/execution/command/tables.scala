...
case class DescribeTableCommand(
  ...
  private def runAsJson(sparkSession: SparkSession): Seq[Row] = {
    ...
    // Add tableName
    addKeyValueToJson(result, "table_name", JString(table.table))

    // If not temp view, add qualified_name
    ...
    if (!catalog.isTempView(table)) {
      ...
      addKeyValueToJson(result, "catalog_name", parse(catalogNameArr))
      addKeyValueToJson(result, "database_name", parse(databaseNameArr))
...
//      }
  ...
  private def addKeyValueToJson(buffer: ArrayBuffer[Row], key: String, value: JValue): Unit = {
    ...
    val updatedJson = currentJson merge JObject(key -> value)
    val jsonString = compact(render(updatedJson))

    if (buffer.isEmpty) {
      buffer += Row(normalizeJson(jsonString), "", "")
    } else {
      buffer(0) = Row(normalizeJson(jsonString), "", "")
    }
  }
  ...
  private def describeFormattedTableInfoJson(
    ...
    val excludedTableInfo = Set("catalog", "schema", "database", "table")
