...
abstract class MergeIntoTableSuiteBase
  ...
  test("Merge metrics with matched clause") {
    withTempView("source") {
      ...
      assert(commitProps("numSourceRows") === "3")
  ...
  test("Merge metrics with matched, not matched, and not matched by source clauses: update") {
    withTempView("source") {
      ...
      val table = catalog.loadTable(ident)
      val commitProps = table.asInstanceOf[InMemoryTable].commits.last.properties
      assert(commitProps("numSourceRows") === "4")
      assert(commitProps("merge.numTargetRowsCopied") === (if (deltaMerge) "0" else "3"))
      assert(commitProps("merge.numTargetRowsInserted") === "1")
      assert(commitProps("merge.numTargetRowsUpdated") === "2")
      assert(commitProps("merge.numTargetRowsDeleted") === "0")
  ...
  test("Merge metrics with numSourceRows for empty source") {
    withTempView("source") {
      ...
      val table = catalog.loadTable(ident)
      val commitProps = table.asInstanceOf[InMemoryTable].commits.last.properties
      assert(commitProps("numSourceRows") === "-1") // if no numSourceRows, should be -1
      ...
      case (withSchemaEvolution, schemaEvolutionEnabled) =>
        withTempView("source") {
          ...
          if (withSchemaEvolution && schemaEvolutionEnabled) {
            ...
            assert(
              e.getMessage.contains("A column, variable, or function parameter with name " +
    ...
    Seq((true, true), (false, true), (true, false)).foreach {
      case (withSchemaEvolution, schemaEvolutionEnabled) =>
        withTable(tableNameAsString) {
          withTempView("source") {
            createAndInitTable(
              "pk INT NOT NULL, salary SMALLINT, dep STRING",
              """{ "pk": 1, "salary": 100, "dep": "hr" }
                |{ "pk": 2, "salary": 200, "dep": "finance" }
            ...
            val sourceRows = Seq((1, 50000, "hr"), (4, 40000, "sales"), (5, 500, "marketing"))
            sourceRows.toDF("pk", "salary", "dep").createOrReplaceTempView("source")
      ...
      withTable(tableNameAsString) {
        withTempView("source") {
          ...
          createAndInitTable(
            ...
            """{ "pk": 1, "employee": { "salary": 50000, "details":
              ...
              """{ "pk": 2, "employee": { "salary": 60000, "details":
                |{ "bonus": 6000, "years": 3 } }, "dep": "finance" }""".stripMargin
                .replace("\n", ""))
          ...
          if (withSchemaEvolution) {
            ...
            val tableSchema = sql(s"SELECT * FROM $tableNameAsString").schema
            val employeeField = tableSchema
              .find(_.name == "employee")
              .get
              .dataType
              .asInstanceOf[StructType]
            val detailsField = employeeField
              .find(_.name == "details")
              .get
              .dataType
              .asInstanceOf[StructType]
            ...
            assert(
              exception.getMessage.contains(
                "Fail to assign a value of \"BIGINT\" type " +
      ...
      withTable(tableNameAsString) {
        withTempView("source") {
          ...
          if (withSchemaEvolution) {
            ...
            assert(
              exception.getMessage.contains("Fail to assign a value of \"BIGINT\" type " +
          ...
          val sourceTableSchema = StructType(
            Seq(
              ...
              StructField(
                "data",
                StructType(
                  Seq(StructField("value", IntegerType), StructField("timestamp", LongType)))),
              StructField("dep", StringType)))

          val data =
            Seq(Row(1, Row(150, 1634567890L), "test"), Row(3, Row(300, 1634567900L), "new"))
      ...
      withTempView("source") {
        ...
        if (withSchemaEvolution) {
          ...
          checkAnswer(
            sql(s"SELECT * FROM $tableNameAsString"),
            Seq(
              Row(1, Row(-1, Row(Seq(-1), Map("k" -> "v"), false)), "hr"),
      ...
      withTempView("source") {
        ...
        if (withSchemaEvolution) {
          ...
          checkAnswer(
            sql(s"SELECT * FROM $tableNameAsString"),
            Seq(
              Row(1, Row(10, Row(Seq(3, 4), Map("c" -> "d"), false)), "sales"),
  ...
  test("merge into schema evolution replace column with nested field and set all columns") {
    Seq(true, false).foreach { withSchemaEvolution =>
      withTempView("source") {
        ...
        assert(
          exception.getMessage.contains("Cannot find data for the output column `s`.`c2`.`a`"))
      }
      sql(s"DROP TABLE IF EXISTS $tableNameAsString")
    }
  }
      ...
      withTempView("source") {
        ...
        if (withSchemaEvolution) {
          ...
          checkAnswer(
            sql(s"SELECT * FROM $tableNameAsString"),
            Seq(
              Row(0, Array(Row(1, "a", null), Row(2, "b", null)), "sales"),
              Row(1, Array(Row(10, "c", true), Row(20, "d", false)), "hr"),
              Row(2, Array(Row(30, "d", false), Row(40, "e", true)), "engineering")))
        } else {
          val exception = intercept[org.apache.spark.sql.AnalysisException] {
  ...
  test("merge into schema evolution add column for struct in map and set all columns") {
    Seq(true, false).foreach { withSchemaEvolution =>
      withTempView("source") {
        ...
        spark
          .createDataFrame(spark.sparkContext.parallelize(sourceData), sourceTableSchema)
          .createOrReplaceTempView("source")
  ...
  test("merge into empty table with NOT MATCHED clause schema evolution") {
    Seq(true, false) foreach { withSchemaEvolution =>
      withTempView("source") {
        ...
        val sourceRows =
          Seq((1, 100, "hr", true), (2, 200, "finance", false), (3, 300, "hr", true))
        ...
        if (withSchemaEvolution) {
          checkAnswer(
            sql(s"SELECT * FROM $tableNameAsString"),
            Seq(Row(1, 100, "hr", true), Row(2, 200, "finance", false), Row(3, 300, "hr", true)))
        } else {
          checkAnswer(
            sql(s"SELECT * FROM $tableNameAsString"),
            Seq(Row(1, 100, "hr"), Row(2, 200, "finance"), Row(3, 300, "hr")))
        }
